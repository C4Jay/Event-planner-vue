    ...
    <v-container grid-list-lg>
        <v-layout row wrap >
            <v-flex xs12 sm12 md3 v-for="item in items" :key="item.id">
                <v-card v-if="location == '' || location == item.location" elevation="19">
                    <v-responsive>
                    ...
                    <v-btn block color="green" :to="'/catering/' + item.id" >view</v-btn>
                </v-card>
            </v-flex>
...
export default {
    data () {
        return {
           ...
           valid: false,
      location: '',
      categories: ['Luxury', 'Vintage', 'Both'],
   ...
   /*  mounted: {
    ...
    computed: {
        items () {
            return this.$store.getters.catering
            // console.log(this.$store.getters.hotels)
        }
    },
