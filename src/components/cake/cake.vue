    ...
    <v-container grid-list-lg>
        <v-layout row wrap >
            <v-flex xs12 sm12 md3 v-for="item in items" :key="item.id">
                <v-card :to="'/cake/' + item.id" v-if="(location == '' || location == item.location ) && (category == '' || item.category == 'All kinds' || category == item.category )" elevation="19">
                    <v-responsive>
...
export default {
    data () {
        return {
           ...
           valid: false,
      location: '',
      categories: ['Structures','Wedding Cake','Cupcake','Structures & Wedding Cake','Structures & Cupcake', 'Cupcake & Wedding Cake', 'All kinds'],
