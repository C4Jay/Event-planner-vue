    ...
    <v-container grid-list-lg>
        <v-layout row wrap class="desc">
            <v-flex xs12 sm12 md3 v-for="item in items" :key="item.id">
                <v-card :to="'/bridalwear/' + item.id" v-if="(item.verified == 'yes' && location == '' || location == item.location ) && (category == '' || item.category == 'All kinds' || category == item.category )" elevation="19">
                    <v-responsive>
                    ...
                    <v-card-text>
                        <div>
                            ...
                            <h3 class="text-weight-black">Wedding dresses</h3>
                            <pre class="font-weight-light">{{item.weddinglist}}</pre>
                            
                              

                            <h3 class="text-weight-black">Homecoming dresses</h3>
                            <pre class="font-weight-light">{{item.homecominglist}}</pre>
...
export default {
    data () {
        return {
           ...
           valid: false,
      location: '',
      categories: ['Groom`s wear - Traditional', 'Groom`s wear - Western', 'Bridal wear - Traditional', 'Bridal wear - Western', 'All kinds'],
   ...
   /*  mounted: {
    ...
    computed: {
        items () {
            return this.$store.getters.bridalwears
            // console.log(this.$store.getters.hotels)
        }
    },
